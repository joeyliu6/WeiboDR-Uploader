...
3. ✅ **无并发限制**: 所有图床同时上传，提升用户体验和进度可见性
4. ✅ **按序选择原则**: 按选择顺序中第一个成功的图床作为 primary（用户可控）
...
- 所有图床并发上传（无并发限制）
- 使用 `Promise.all` 并行上传
- 提取按选择顺序第一个成功的作为 `primaryService`
- 返回所有结果（成功+失败）
...
1. **并行上传资源占用**:
   - 所有图床同时上传对带宽和内存的影响在可接受范围内
   - **实际测试**: 4-8 个图床并发上传无明显性能问题
   - **未来优化**: 如有需要可添加用户自定义并发数配置
