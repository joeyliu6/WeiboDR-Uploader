  ...
  try {
    ...
    try {
      ...
      initializeTheme().then(() => {
        console.log('[初始化] ✓ 主题系统已初始化');
      }).catch(error => {
        console.error('[初始化] 主题系统初始化失败:', error);
      });
    } catch (error) {
      console.error('[初始化] 主题系统初始化失败:', error);
    }
    ...
    if (backupView) {
      try {
        ...
        backupApp.use(ToastService);
        backupApp.use(ConfirmationService);
        backupApp.directive('tooltip', Tooltip);
